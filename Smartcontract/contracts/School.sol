...
contract SchoolSystem is ERC721, Ownable {
    ...
    function createClassSession(
        ...
        uint sessionId = uint(
            keccak256(abi.encodePacked(block.timestamp, msg.sender, _courseId))
        );
    ...
    function markAttendance(uint _sessionId) external onlyStudent {
        ...
        require(
            courses[session.courseId].students[msg.sender],
        ...
        returns (uint[] memory)
    {
        ...
        for (uint i = 1; i <= courseCount; i++) {
            if (
                courses[i].lecturer == msg.sender && courses[i].sessionId != 0
            ) {
